...
### 2 — Generate your config (run locally first)
```bash
npm install
npm run setup    # writes .env with auto-generated NEXTAUTH_SECRET, ADMIN_PASSWORD, WEBHOOK_SECRET
```
Open the generated `.env`, fill in the `CHANGE_ME` values, then copy all variables into Railway.

### 3 — Web service (Next.js)
Railway auto-detects `nixpacks.toml`. Set these environment variables in the Railway dashboard for the web service:

| Variable              | Value                                         |
|-----------------------|-----------------------------------------------|
| `NEXTAUTH_SECRET`     | Auto-generated by `npm run setup`             |
| `NEXTAUTH_URL`        | `https://<your-railway-domain>`               |
| `ADMIN_USERNAME`      | `admin`                                       |
| `ADMIN_PASSWORD`      | Auto-generated by `npm run setup`             |
| `MAIL_DOMAIN`         | Your inbox domain, e.g. `mail.example.com`    |
| `WEBHOOK_SECRET`      | Auto-generated by `npm run setup`             |
| `WEBSITE_URL`         | `https://yourdomain.com`                      |
| `IMAP_HOST`           | Your Mailcow IMAP host                        |
| `IMAP_USER`           | Catch-all mailbox address                     |
| `IMAP_PASS`           | Catch-all IMAP password                       |
| `MAILCOW_URL`         | *(optional)* Your Mailcow base URL            |
| `MAILCOW_API_KEY`     | *(optional)* Mailcow read+write API key       |

### 4 — Custom domain
1. In Railway → your web service → **Settings → Domains → Add Custom Domain**
2. Enter your domain, copy the `CNAME` target into your DNS provider
3. Wait for propagation (< 5 min on Cloudflare)
4. Railway auto-provisions a Let's Encrypt TLS certificate
5. Update `NEXTAUTH_URL` and `WEBSITE_URL` to your custom domain
...
| `DISCORD_BOT_PREFIX` | `.` (or `!` if you prefer)                   |
| `DISCORD_OWNER_IDS`  | Your Discord user ID                          |
| `WEBSITE_URL`        | `https://yourdomain.com`                      |
| `BACKUP_CHANNEL_ID`  | *(optional)* Channel for DB backups           |

### 6 — Bot first-time setup
...
```bash
npm install
npm run setup        # auto-generates .env with random secrets — edit the CHANGE_ME values
npm run dev          # Next.js on http://localhost:3000
npm run bot:start    # Discord bot (separate terminal)
```

### Re-generate secrets (overwrites .env)
```bash
npm run setup -- --force
```
